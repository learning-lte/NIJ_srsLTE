...
////////////////////////////
// Keeps track of how many frames have been written to file
...
void cc_worker::work_ul(const srslte_ul_sf_cfg_t& ul_sf_cfg, stack_interface_phy_lte::ul_sched_t& ul_grants)
{
  ...
  // Process UL signal
  /////////////////////////////////////////////
  //Only copy the third symbol (the DMRS symbol)
  cf_t* temp = get_buffer_rx(1);
  //cout << "Symbol Size: " << enb_ul.fft.cfg.symbol_sz << " Subframe Size: " << SRSLTE_SF_LEN(enb_ul.fft.cfg.symbol_sz) << endl;
  //Needs to include symbol and CP
  srslte_enb_ul_raw_copy(&enb_ul, temp, SRSLTE_SF_LEN(enb_ul.fft.cfg.symbol_sz)*sizeof(cf_t), enb_ul.fft.cfg.symbol_sz);
  //srslte_enb_ul_raw_copy(&enb_ul, temp, SRSLTE_SYMBOL_SZ(enb_ul.fft.cfg.symbol_sz, 0)*sizeof(cf_t), enb_ul.fft.cfg.symbol_sz);
  ...
  //////////////////////////////////////////
  free(enb_ul.raw_copy);
  free(enb_ul.raw_copy2);
...
void cc_worker::decode_pusch(stack_interface_phy_lte::ul_sched_grant_t* grants, uint32_t nof_pusch)
{
  ...
  for (uint32_t i = 0; i < nof_pusch; i++) {
    ...
    if (ul_grant.data != nullptr) {
      ...
      /////////////////////////////////////////
      uint32_t csi_size = ul_cfg.pusch.grant.L_prb * SRSLTE_NRE;
      //uint32_t csi_point = SRSLTE_REFSIGNAL_UL_L(1, enb_ul.chest.cell.cp) * enb_ul.chest.cell.nof_prb * SRSLTE_NRE +
                          //ul_cfg.pusch.grant.n_prb[1] * SRSLTE_NRE;
      //cout << "Type: " << typeid(enb_ul.chest_res.ce[csi_point]).name() << " Orig: " << enb_ul.chest_res.ce[csi_point+10] << " After: " << csi_point << endl;
      //cout << "Phone RSRP: " << srslte_convert_power_to_dB(enb_ul.chest_res.snr * enb_ul.chest_res.noise_estimate) << " Phone SNR: "<<  enb_ul.chest_res.snr_db << " Phone Noise Estimate: " << enb_ul.chest_res.noise_estimate_dbm <<  endl;
      write_raw_file(&enb_ul, csi_size, srslte_convert_power_to_dB(enb_ul.chest_res.snr * enb_ul.chest_res.noise_estimate) + 30, tti_rx);
      /////////////////////////////////////////
      // Logging
...
//NIJ Function: Writes two raw antenna buffers, csi, rsrp, and tti to a file
void cc_worker::write_raw_file(srslte_enb_ul_t *q, uint16_t csi_size, float rsrp, uint32_t tti_rx)
{
	FILE *fraw_out;
	fraw_out = fopen("/home/nick/srsLTE/raw_data.txt", "a");
	cf_t temp = (999999.0);
	cf_t* deliminator = &temp;
	cf_t temp1 = (999999.1);
	cf_t* deliminator1 = &temp1;
	cf_t temp2 = (999999.2);
	cf_t* deliminator2 = &temp2;
  cf_t temp3 = (999999.3);
	cf_t* deliminator3 = &temp3;
  cf_t temp4 = (999999.4);
	cf_t* deliminator4 = &temp4;
  cf_t rsrp_converted = (rsrp);
  cf_t* rsrp_pointer = &rsrp_converted;
  cf_t tti_converted = ((float)tti_rx);
  cf_t* tti_pointer = &tti_converted;
  //fwrite(q->raw_copy, sizeof(cf_t), SRSLTE_SYMBOL_SZ(q->fft.cfg.symbol_sz, 0), fraw_out);
  fwrite(q->raw_copy, sizeof(cf_t), SRSLTE_SF_LEN(q->fft.cfg.symbol_sz), fraw_out);
	fwrite(deliminator, sizeof(cf_t), 1, fraw_out);
  //fwrite(q->raw_copy2, sizeof(cf_t), SRSLTE_SYMBOL_SZ(q->fft.cfg.symbol_sz, 0), fraw_out);
  fwrite(q->raw_copy2, sizeof(cf_t), SRSLTE_SF_LEN(q->fft.cfg.symbol_sz), fraw_out);
	fwrite(deliminator1, sizeof(cf_t), 1, fraw_out);
	fwrite(q->chest_res.ce, sizeof(cf_t), csi_size, fraw_out);
	fwrite(deliminator2, sizeof(cf_t), 1, fraw_out);
  fwrite(rsrp_pointer, sizeof(cf_t), 1, fraw_out);
  fwrite(deliminator3, sizeof(cf_t), 1, fraw_out);
  fwrite(tti_pointer, sizeof(cf_t), 1, fraw_out);
  fwrite(deliminator4, sizeof(cf_t), 1, fraw_out);
  frame_counter++;
  //cout << "Number of frames captured: " << frame_counter << " which is TTI=" << tti_rx << endl;
