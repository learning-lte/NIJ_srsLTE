...
//NIJ Function: Copies complex sample arrayys to enb members
void srslte_enb_ul_raw_copy(srslte_enb_ul_t *q,  cf_t* ant2_buff, uint32_t size, uint32_t symbol_size)
{
  //Calculate start of 4th symbol (DMRS) 
  uint32_t start = 0;
  /*for (int l=0; l < 4; l++){
      start = start + SRSLTE_CP_LEN_NORM(l, symbol_size);
  }*/

  //Copies sample buffers from both antennae
	q->raw_copy = (cf_t*) srslte_vec_malloc(size);
	bzero(q->raw_copy, size);
	memcpy(q->raw_copy, q->fft.cfg.in_buffer+start, size);
  q->raw_copy2 = (cf_t*) srslte_vec_malloc(size);
	bzero(q->raw_copy2, size);
	memcpy(q->raw_copy2, ant2_buff+start, size);
}
